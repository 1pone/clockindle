...
var KEY_LUNAR = "c8be368a035acdf1"; // APIs

var API_HITOKOTO = "https://v1.hitokoto.cn?encode=json&charset=utf-8"; // var API_IP_INFO = "https://ipapi.co/"
...
var API_WEIBO = "https://tenapi.cn/resou/"; // 组件容器
// TODO 添加组件刷新频率

var TOP_MODE = ["nonetop", "hitokoto", "poem", "weibo"];
var BOTTOM_MODE = ["nonebtm", "weather"];
var BG_MODE = ["none", "dark", "auto", "pic"]; // 默认配置项
...
var autoModeImg = "&#xe8e3"; // 创建XMLHttpRequest对象
...
function clock(autoMode) {
  ...
  var minutes = date.getMinutes(); // 深浅色模式标示

  var lightMode = true; // 自动模式
  ...
  if (!dd_data || dd !== dd_data) {
    ...
    document.getElementById("date").innerHTML = dateString + " " + weekString; // 获取农历日期及节假日
  ...
  xhr.onreadystatechange = function () {
    if (this.readyState == 4) {
      ...
      if (data.code === "200") {
        ...
        var weaInfo = "<div>" + city + "当前天气" + "</div>" + "<div>体感温度：" + wea_now.feelsLike + "&#8451;</div>" + "<div>湿度：" + wea_now.humidity + "%</div>" + "<div>风向：" + wea_now.windDir + "</div>" + "<div>风速：" + wea_now.windScale + "级 " + wea_now.windSpeed + "km/h</div>" + "<div>更新时间：" + wea_now.obsTime.match(/T(.+)\+/)[1];
        "</div>";
    ...
    if (this.readyState == 4) {
      ...
      if (data.data === 200) {
        ...
        for (var i = 0; i < weibo_num; i++) {
          var index = i + 1;
          hot_word.innerHTML += "<li>" + index + ". " + weibo_data[i].name + "</li>"; // alapi
...
function picture() {
  console.log("picture update"); // var xhr = createXHR();
  // xhr.open(
  //   "GET",
...
function changeBgMode() {
  ...
  var page = document.getElementsByClassName("page")[0];
  var backdrop = document.getElementById("settings_backdrop");
  ...
  if (bg_mode === 0) {
    ...
    backdrop.style.background = "url('../css/images/pixel_black.png')";
  } else if (bg_mode === 1) {
    // dark bg
    pageClasses.remove("light");
    pageClasses.add("dark");
    backdrop.style.background = "url('../css/images/pixel_white.png')";
  } else if (bg_mode === 2) {
    // auto bg
    var date = new Date();
