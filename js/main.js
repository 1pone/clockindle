...
function showScreenSize() {
    ...
    console.log(w, h);
    document.getElementById("screensize").innerHTML = "屏幕分辨率的宽：" + w + "</br>屏幕分辨率的高：" + h + "</br>getTimezoneOffset: " + Boolean(new Date().getTimezoneOffset)
}
    ...
    xhr.onreadystatechange = function() {
        if (this.readyState == 4) {
            ...
            if (data.code === 200) {
                ...
                date.setMinutes(date.getMinutes() + utc8DiffMinutes);
                var hour = date.getHours();
                if (hour > nightHour || hour < morningHour) { img = imgs[1] }
                var weaImg = '<span class="iconfont">' + img + "</span>" + "<div>天气：" + weather_data.weather + "</div>";
                var weaTemp = '<div class="tempNum">' + weather_data.temp + '<div class="symbol">&#8451;</div></div>' + "<div>当前气温</div>";
                ...
                var weaInfo = "<div>最高/低气温：" + highTemp + "/" + lowTemp + "&#8451;</div>" + "<div>湿度：" + weather_data.humidity + "</div>" + "<div>空气质量：" + air + airLevel + "</div>" + "<div>风向：" + weather_data.wind + "</div>" + "<div>风速：" + weather_data.wind_speed + " " + weather_data.wind_scale + "</div>" + "<div>更新时间：" + updateTime + "</div>";
        ...
        if (this.readyState == 4) {
            ...
            if (data.code === 200) {
                ...
                for (var i = 0; i < weibo_num; i++) {
                    var index = i + 1;
                    hot_word.innerHTML += "<li>" + index + ". " + weibo_data[i].hot_word + "</li>";
                    hot_word_num.innerHTML += "<li>" + weibo_data[i].hot_word_num + "</li>"
                }
            } else {
...
function changeMode(pos) {
    ...
    if (pos_mode !== 0) {
        if (!eval(POS_MODE[pos_mode] + "_data")) { eval(POS_MODE[pos_mode] + "()") }
        eval(POS_MODE[pos_mode] + '_timer = setInterval(POS_MODE[pos_mode] + "()", 60 * 1000 * 20)');
        console.log(POS_MODE[pos_mode] + "_timer created")
    }
    for (var i = 0; i < POS_MODE.length; i++) { document.getElementsByClassName(POS_MODE[i] + "_container")[0].style.display = "none" }
    document.getElementsByClassName(POS_MODE[pos_mode] + "_container")[0].style.display = "block"
}

function changeTopMode() { changeMode("top") }

function changeBottomMode() { changeMode("bottom") }
